...
namespace serialization
{
    ...
    void Json::stringify(string &out, JsonFormat strategy) const
    {
        switch (strategy)
        {
        case JsonFormat::BINARY_STANDARD:
        case JsonFormat::BINARY_EXTENTED:
        {
            CborStream cbs;
            cbs << *this;
            out = cbs.data();
    ...
    public:
        StringParser(const string &content, string &error_msg,
                     const JsonFormat &parser_type = JsonFormat::STRING_STANDARD)
        ...
        friend Json Json::parse(const string &in, string &err, JsonFormat strategy);
    ...
    private:
        ...
        {
            consume_whitespace();
            if (strategy == JsonFormat::STRING_COMMENTS)
            {
    ...
    Json Json::parse(const string &in, string &err, JsonFormat strategy)
    {
        if (strategy == JsonFormat::BINARY_STANDARD)
        {
