...
struct test_encoder_stream_io
{
  ...
  test_encoder_stream_io(int _a, int _b, double _c, std::string _d)
      : a(_a), b(_b), c(_c), d(_d) {}
...
REFLECT(TestReflect::Point, FIELD(x), FIELD(y), FIELD(dd), FIELD(str))

REFLECT(TestReflect::Rect, FIELD(specialPoint), FIELD(plist), FIELD(color))
...
void jsonp_test()
{
  ...
  string err_comment;
  auto json_comment =
      Json::parse(comment_test, err_comment, Json::Format::STRING_COMMENTS);
  ...
  auto json_failing_comment = Json::parse(
      failing_comment_test, err_failing_comment, Json::Format::STRING_COMMENTS);
  ...
  TEST_ASSERT(std::memcmp(uni[0].string_value().data(), utf8, sizeof utf8) ==
              0);
...
void cborp_test()
{
  ...
  auto j = Json::parse(array_string, err, Json::Format::BINARY_STANDARD);
  TEST_ASSERT(err.empty());
  TEST_ASSERT(j.type() == Json::ARRAY);
  TEST_ASSERT(j.array_items().empty());

  array_string = bytes2string({0x81, 0xf6});
  j = Json::parse(array_string, err, Json::Format::BINARY_STANDARD);
  TEST_ASSERT(err.empty());
  TEST_ASSERT(j.type() == Json::ARRAY);
  ...
  array_string =
      bytes2string({0x82, 0x67, 0x73, 0x74, 0x72, 0x69, 0x6E, 0x67, 0x41, 0x67,
                    0x73, 0x74, 0x72, 0x69, 0x6E, 0x67, 0x42});
  j = Json::parse(array_string, err, Json::Format::BINARY_STANDARD);
  TEST_ASSERT(err.empty());
  TEST_ASSERT(j.type() == Json::ARRAY);
  ...
  array_string =
      bytes2string({0x83, 0xFB, 0x40, 0x11, 0xD7, 0x0A, 0x3D, 0x70, 0xA3, 0xD7,
                    0xFB, 0xC1, 0x2E, 0x84, 0x7F, 0xCC, 0xCC, 0xCC, 0xCD, 0xFB,
                    0x3E, 0xB4, 0xB3, 0xFD, 0x59, 0x42, 0xCD, 0x96});
  j = Json::parse(array_string, err, Json::Format::BINARY_STANDARD);
  TEST_ASSERT(err.empty());
  TEST_ASSERT(j.type() == Json::ARRAY);
  ...
  j = Json::parse(object_string, err, Json::Format::BINARY_STANDARD);
  TEST_ASSERT(err.empty());
  ...
  object_string =
      bytes2string({0xa2, 0x61, 0x61, 0x01, 0x61, 0x62, 0x82, 0x02, 0x03});
  j = Json::parse(object_string, err, Json::Format::BINARY_STANDARD);
  TEST_ASSERT(err.empty());
  TEST_ASSERT(j.type() == Json::OBJECT);
  EXPECT_STRING_EQUAL(j.stringify().c_str(), "{\"a\": 1, \"b\": [2, 3]}");

  object_string = bytes2string({0xa5, 0x61, 0x61, 0x61, 0x41, 0x61, 0x62,
...
void combo_test()
{
       ...
       {"1.json", "2.json", "3.json", "4.json", "5.json", "6.json"})
  {
    ...
    TEST_ASSERT(err.empty());
    ifs.close();
    ifs.open(std::string("../json.org/") + filename + ".cbor",
             std::ios_base::binary);
    string str2((std::istreambuf_iterator<char>(ifs)),
                std::istreambuf_iterator<char>());
    auto j_binary = Json::parse(str2, err, Json::Format::BINARY_STANDARD);
    TEST_ASSERT(err.empty());
...
void codec_test()
{
  ...
  PREPARE_TEST
  {
    ...
    EXPECT_CBOREQ(cbs.GetData(),
                  "02 0e 18 19 18 38 18 f1 34 38 7b 24 39 13 fb 39 61 44 ");
  ...
  PREPARE_TEST
  {
    ...
    EXPECT_CBOREQ(cbs.GetData(),
                  "1a b2 d0 5e 00 1b 00 00 00 69 54 3b 27 73 1b 00 00 0f ee 24 "
                  "0e 45 7a 1b "
  ...
  PREPARE_TEST
  {
    ...
    EXPECT_CBOREQ(cbs.GetData(),
                  "fb 3f 48 b5 02 ab ab ea d5 fb 40 41 0f 5c 28 f5 c2 8f fb 40 "
                  "1f f2 23 ce 10 6e b8 fb "
  ...
  PREPARE_TEST
  {
    ...
    EXPECT_CBOREQ(cbs.GetData(), "68 30 2e 30 30 30 37 35 34 67 33 61 64 34 66 "
                                 "31 32 65 62 68 64 73 66 69 30 78 61 73 68 64 "
  ...
  PREPARE_TEST
  {
    ...
    EXPECT_CBOREQ(cbs.GetData(), "68 30 2e 30 30 30 37 35 34 67 33 61 64 34 66 "
                                 "31 32 65 62 68 64 73 66 69 30 78 61 73 68 64 "
                                 "67 6f 78 60 66 77 65 72 74 74 74 68 63 65 73 "
  ...
  PREPARE_TEST
  {
    ...
    EXPECT_CBOREQ(cbs.GetData(),
                  "85 01 02 03 04 05 85 01 02 03 04 05 83 fb 40 14 39 58 10 62 "
                  "4d d3 fb 40 14 39 58 10 62 4d d3 fb 40 14 39 58 "
                  "10 62 4d d3 84 64 63 65 68 69 67 33 32 38 34 36 64 65 68 71 "
                  "75 65 75 64 62 76 66 6c 25 5e 34 35 32 34 33 2a "
  ...
  PREPARE_TEST
  {
    ...
    cbs << mp1 << mp3;
    EXPECT_CBOREQ(cbs.GetData(),
                  "a3 01 02 02 02 03 18 38 a3 65 74 65 73 74 31 83 1a 00 0b f1 "
                  "90 18 7b 3a 00 05 3c c9 65 74 65 73 74 32 83 1a "
                  "00 01 7e b1 39 5b 7f 39 30 48 65 74 65 73 74 33 83 19 04 bc "
                  "39 03 dc 00 ");
    cbs << pp;
  }
  ...
  PREPARE_TEST
  {
    ...
    EXPECT_CBOREQ(cbs.GetData(),
                  "69 5b 31 2c 5b 31 2c 32 5d 5d 65 63 65 73 68 69 83 fb 3f f3 "
                  "33 33 33 33 33 33 fb 40 02 66 66 66 66 "
  ...
  {
    std::stringstream ss;
    EXPECT_STRING_EQUAL(enum_to_string(TestEnum::Foo::NestedEnum::A).c_str(),
                        "fa");
    EXPECT_STRING_EQUAL(enum_to_string(TestEnum::Foo::NestedEnum::B).c_str(),
                        "fb");
...

  {
    ...
    cbs << j;
    string err;
    auto j2 = Json::parse(cbs.GetData(), err, Json::Format::BINARY_STANDARD);
    EXPECT_EQ(j, j2);
...

  {
    ...
    TestReflect::Rect rect1{p1, std::vector<TestReflect::Point>{p1, p2},
                            0xffff};
    TestReflect::Rect rect2{p3, std::vector<TestReflect::Point>{p2, p3},
                            0xffee};
    TestReflect::Box box;
