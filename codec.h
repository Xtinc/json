...
struct EpochTimeUs {
  ...
  void SetTime(const std::string &t) {
    std::tm time = {};
    sscanf(t.c_str(), "%04d-%02d-%02dT%02d:%02d:%02d", &time.tm_year,
  ...
  std::string PrintEpochTime() const {
    ...
    snprintf(buff, 30, "%04d-%02d-%02dT%02d:%02d:%02d.%06" PRId64 "Z",
             (1900 + time_info.tm_year) % 10000u, (1 + time_info.tm_mon) % 100u,
             time_info.tm_mday, time_info.tm_hour, time_info.tm_min,
             time_info.tm_sec, m_ullEpochTimeUs % 1000000);
    return buff;
  }
};
...
public:
  ...
  enum Type { NUL, FLOAT, INTEGER, BOOL, STRING, ARRAY, OBJECT };
  ...
  template <class T,
            enable_if_t<is_arithmetic_integer_number<T>::value, int> = 0>
  Json(T value) : m_ptr(make_unique<details::JsonInt>(value)) {}

  template <class T,
            enable_if_t<std::is_floating_point<decay_t<T>>::value, int> = 0>
  Json(T value) : m_ptr(make_unique<details::JsonDouble>(value)) {}

  template <class T, enable_if_t<std::is_same<T, bool>::value, int> = 0>
  Json(T value) : m_ptr(make_unique<details::JsonBoolean>(value)) {}

  template <class T,
            enable_if_t<std::is_same<decay_t<T>, std::string>::value, int> = 0>
  Json(T &&value)
      : m_ptr(make_unique<details::JsonString>(std::forward<T>(value))) {}

  template <class T,
            enable_if_t<std::is_same<decay_t<T>, const char *>::value, int> = 0>
  Json(T &&value)
      : m_ptr(make_unique<details::JsonString>(std::forward<T>(value))) {}

  template <class T,
            enable_if_t<std::is_same<decay_t<T>, array>::value, int> = 0>
  Json(T &&values)
      : m_ptr(make_unique<details::JsonArray>(std::forward<T>(values))) {}

  template <class T,
            enable_if_t<std::is_same<decay_t<T>, object>::value, int> = 0>
  Json(T &&values)
      : m_ptr(make_unique<details::JsonObject>(std::forward<T>(values))) {}
  ...
  template <class R, enable_if_t<is_reflected_object<R>::value, int> = 0>
  Json(const R &r) {
    std::string err;
    *this = json_from_object(r, err);
  }
  ...
  // Serialize.
  void stringify(std::string &out,
                 Json::Format strategy = STRING_STANDARD) const;

  std::string
  stringify(Json::Format strategy = Json::Format::STRING_STANDARD) const {
    std::string out;
    stringify(out, strategy);
    return out;
  }

  // De-serialize. If parse fails, return Json() and assign an error message to
  // err.
...
inline static void dump(NullStruct, std::string &out) { out += "null"; }
...
protected:
  ...
  virtual const std::string &string_value() const {
    return null_object.empty_string;
  }

  virtual const Json::array &array_items() const {
    return null_object.empty_vector;
  }

  // array
  virtual const Json &operator[](size_t i) const {
    return null_object.empty_json;
  }

  virtual Json &operator[](size_t i) {
    throw std::out_of_range("non-const reference to null json object");
  }
...
protected:
  ...
  void dump(std::string &out) const override {
    serialization::details::dump(m_value, out);
  }
...
class JsonArray final : public Value<Json::ARRAY, Json::array> {
  ...
  Json &operator[](size_t i) override { return m_value[i]; }

  const Json &at(size_t i) const override { return m_value.at(i); }

  Json &at(size_t i) override { return m_value.at(i); }
...
class JsonObject final : public Value<Json::OBJECT, Json::object> {
  ...
  Json &at(const std::string &key) override { return m_value.at(key); }
...
public:
  StringParser(const std::string &content, std::string &error_msg,
      ...
      : str(content), i(0), err(error_msg), failed(false),
...
private:
   ...
   */
  Json fail(std::string &&msg) { return fail(move(msg), Json()); }
...
public:
  CborParser(const std::string &in, std::string &err_msg)
      : status(DecodeStatus::START), curidx(0), curlen(0), instant_num(0),
        str(in), err(err_msg) {}
...
private:
  ...
  bool has_bytes(size_t n) { return curidx + n <= str.size(); }

  unsigned char get_byte() { return str.at(curidx++); }

  unsigned short get_uint16() {
    unsigned short value = static_cast<unsigned short>(get_byte()) << 8 |
                           static_cast<unsigned short>(get_byte());
    return value;
  }
  ...
  unsigned long long get_uint64() {
    unsigned long long value =
        (static_cast<unsigned long long>(get_byte()) << 56) |
...
private:
    ...
    case 4:
      // todo :overflow
      {
        ...
        if (temp <= (uint32_t)std::numeric_limits<int>::max()) {
          ...
          instant_num =
              fail("exceed integer range", std::numeric_limits<int>::max());
        }
      }
      break;
      // u64
    ...
    case 4:
      // todo :overflow
      {
        ...
        if (temp <= (uint32_t)std::numeric_limits<int>::max()) {
          ...
          result =
              fail("exceed integer range", std::numeric_limits<int>::min());
        }
      }
      break;
      // u64
  ...
  std::map<std::string, Json> parser_object(int depth) {
    ...
    for (auto i = 0; i < len; ++i) {
      ...
      GetData.insert(
          std::make_pair(key.string_value(), parser_json(depth + 1)));
    }
    return GetData;
  }
  ...
  void decompose_type(int minor_type, int instant_number,
                      DecodeStatus set_status, size_t setlen = 0) {
    if (minor_type <= 0x17) {
      curlen = setlen;
...
private:
  ...
  struct encode_each_in_tuple_helper {
    static void apply(Tuple &&tp, CborStream &cbs, size_t &len) {
      ...
      encode_each_in_tuple_helper<Tuple, I + 1u>::apply(std::forward<Tuple>(tp),
                                                        cbs, len);
    }
  };
  ...
  template <typename F> struct reflect_recur_func;

  template <typename T, typename = void>
  struct encode_each_in_reflected_helper {
    static void apply(T &&obj, CborStream &cbs, size_t &len,
  ...
  template <typename T>
  struct encode_each_in_reflected_helper<
      T, enable_if_t<is_reflected_object<decay_t<T>>::value>> {
    static void apply(T &&obj, CborStream &cbs, size_t &len,
                      const char *fieldName = "") {
      ...
      len += cbs.EncodeTagAndValue(details::ucMap,
                                   std::tuple_size<ReflectedType>::value);
...
public:
  ...
  template <typename T> CborStream &operator<<(const T &t) {
    EncodeData(t);
  ...
  template <typename T> CborStream &operator>>(T &t) {
    m_iErr = 0;
  ...
  std::string GetData() const { return m_vBuf; }
  ...
  void InsertRawDictHeader(int s) {
    ...
    case 4:
      m_vBuf.insert(0, 1,
                    static_cast<char>(details::ucMap + details::ucLength4));
      break;
    case 2:
      m_vBuf.insert(0, 1,
                    static_cast<char>(details::ucMap + details::ucLength4));
  ...
  int GetErrNum() const { return m_iErr; }
...
private:
  ...
  template <typename Type>
  enable_if_t<std::is_unsigned<Type>::value, std::size_t>
  ...
  template <typename Type>
  enable_if_t<std::is_unsigned<Type>::value, std::size_t>
  DecodeTagAndValue(uint64_t &tag, Type &t, int &err) {
    err = 0;
    if (m_pIter == m_pEnd) {
      err = 1;
      return 0;
    ...
    case details::ucLength8:
      ...
      t |= static_cast<Type>(
               reinterpret_cast<const unsigned char &>(*(m_pIter++)))
           << 56;
      t |= static_cast<Type>(
               reinterpret_cast<const unsigned char &>(*(m_pIter++)))
           << 48;
  ...
  size_t EncodeData(const bool &t) {
    return EncodeDirectly(details::ucsimple,
                          t ? details::ucTrue : details::ucFalse);
  }
  ...
  template <typename Type>
  enable_if_t<std::is_enum<Type>::value, std::size_t>
  ...
  size_t EncodeData(const std::tuple<Types...> &t) {
    ...
    auto len = EncodeTagAndValue(details::ucMap,
  ...
  // check const?
  template <typename Type>
  enable_if_t<std::is_unsigned<Type>::value, std::size_t> DecodeData(Type &t,
                                                                     int &err) {
    err = 0;
    auto tag = details::ucUndefined;
  ...
  template <typename Type>
  enable_if_t<std::is_signed<Type>::value, std::size_t>
  EncodeData(const Type &t) {
  ...
  template <typename Type>
  enable_if_t<std::is_signed<Type>::value, std::size_t> DecodeData(Type &t,
                                                                   int &err) {
    err = 0;
    auto tag = details::ucUndefined;
    uint64_t val = 0;
  ...
  template <typename Type>
  enable_if_t<is_signed_container<Type>::value, std::size_t>
  EncodeData(const Type &t) {
  ...
  template <typename Type>
  enable_if_t<is_unsigned_container<Type>::value, std::size_t>
  EncodeData(const Type &t) {
  ...
  template <typename Type>
  enable_if_t<!is_signed_container<Type>::value &&
  ...
  template <typename Type>
  enable_if_t<!std::is_fundamental<Type>::value &&
                  !is_signed_container<Type>::value &&
                  !is_unsigned_container<Type>::value &&
  ...
  template <typename Type>
  enable_if_t<!std::is_fundamental<Type>::value && !std::is_enum<Type>::value &&
                  !is_signed_container<Type>::value &&
                  !is_unsigned_container<Type>::value &&
                  !is_reflected_object<Type>::value &&
...
// Json Construction & Operator.
inline Json::Json() noexcept : m_ptr(make_unique<details::JsonNull>()) {}

inline Json::Json(const Json &json) {
  switch (json.type()) {
  case Type::ARRAY:
...
inline Json::Json(Json &&json) noexcept : m_ptr(move(json.m_ptr)) {}

inline Json::Json(std::nullptr_t) noexcept
    : m_ptr(make_unique<details::JsonNull>()) {}

inline Json::Json(const std::initializer_list<Json> &l) {
  auto is_an_object = std::all_of(l.begin(), l.end(), [](const Json &ele) {
    return ele.type() == Json::ARRAY && ele.array_items().size() == 2 &&
           ele.array_items().front().type() == Json::STRING;
  });
  if (is_an_object) {
    // the initializer list is a list of pairs -> create object
    Json::object obj;
    for (const auto &ele : l) {
      const auto &pair = ele.array_items();
      obj.emplace(pair.front().string_value(), pair.back());
    }
    m_ptr = make_unique<details::JsonObject>(obj);
  } else {
    // the initializer list describes an array -> create array
    m_ptr = make_unique<details::JsonArray>(array(l.begin(), l.end()));
  }
}

inline Json &Json::operator=(const Json &json) {
  switch (json.type()) {
  case Type::ARRAY:
    m_ptr = make_unique<details::JsonArray>(json.array_items());
    break;
...
inline Json &Json::operator=(Json &&json) noexcept {
...
inline Json &Json::operator[](size_t i) { return (*m_ptr)[i]; }

inline const Json &Json::at(size_t i) const { return (*m_ptr).at(i); }

inline Json &Json::at(size_t i) { return (*m_ptr).at(i); }

inline const Json &Json::operator[](const std::string &key) const {
  return (*m_ptr)[key];
}

inline Json &Json::operator[](const std::string &key) { return (*m_ptr)[key]; }

inline const Json &Json::at(const std::string &key) const {
  return (*m_ptr).at(key);
}

inline Json &Json::at(const std::string &key) { return (*m_ptr).at(key); }

inline bool Json::has_shape(const shape &types, std::string &err) const {
  if (!is_object()) {
...
Json::parse_multi(const std::string &in,
...
inline void Json::stringify(std::string &out, Json::Format strategy) const {
  switch (strategy) {
